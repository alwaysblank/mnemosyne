...
class Mnemosyne
{
	...
	private function loadFile($location)
	{
		if (!file_exists($location)) :
			throw new MnemosyneException(sprintf("The file <code>%s</code> does not exist, or is inaccessible.\n", $location));
		endif;
	...
	private function getDefault($key)
	{
		if (!isset($this->defaults[$key])) :
			throw new MnemosyneException(sprintf("The key <code>%s</code> does not exist.\n", $key));
	 ...
	 * 
	 * @param 		string 							$key
	 * @param 		string|array|int 		$override
	...
	public function remember($key, $override, $validate = false)
	{
		if (!(is_string($override) || is_int($override) || is_array($override))) :
			throw new MnemosyneException("The supplied override is not an acceptable type (string, int, or array).\n");
		endif;

		if (!is_string($key)) :
			throw new MnemosyneException("The supplied key is not a string.\n");
		endif;

		if(!$this->checkKey($key)) :
			throw new MnemosyneException(sprintf("The key %s is not a valid key (only alphanumeric and underscores allowed).\n", $key));
		endif;
